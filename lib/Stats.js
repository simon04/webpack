...
const d = (v, def) => typeof v !== "undefined" ? v : def;
...
class Stats {
	...
	toJson(options, forToString) {
		...
		const showPerformance = dd(options.performance, true);
		const showHash = dd(options.hash, true);
		const showVersion = dd(options.version, true);
		const showTimings = dd(options.timings, true);
		const showAssets = dd(options.assets, true);
		const showEntrypoints = dd(options.entrypoints, !forToString);
		const showChunks = dd(options.chunks, !forToString);
		const showChunkModules = dd(options.chunkModules, !forToString);
		const showChunkOrigins = dd(options.chunkOrigins, !forToString);
		const showModules = dd(options.modules, true);
		const showDepth = dd(options.depth, !forToString);
		const showCachedModules = dd(options.cached, true);
		const showCachedAssets = dd(options.cachedAssets, true);
		const showReasons = dd(options.reasons, !forToString);
		const showUsedExports = dd(options.usedExports, !forToString);
		const showProvidedExports = dd(options.providedExports, !forToString);
		const showChildren = dd(options.children, true);
		const showSource = dd(options.source, !forToString);
		const showErrors = dd(options.errors, true);
		const showErrorDetails = dd(options.errorDetails, !forToString);
		const showWarnings = dd(options.warnings, true);
		const showPublicPath = dd(options.publicPath, !forToString);
		const excludeModules = [].concat(d(options.exclude, [])).map(item => {
	...
	toString(options) {
		...
		const useColors = typeof options.colors !== "undefined" ? options.colors : false;
